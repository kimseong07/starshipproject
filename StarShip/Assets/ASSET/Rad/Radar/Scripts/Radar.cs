...
public class Radar : MonoBehaviour {
    ...
    private void Update() {
        ...
        radar.localPosition = new Vector3(player.localPosition.x, player.localPosition.y, 0);
        ...
        foreach (RaycastHit2D raycastHit2D in raycastHit2DArray) {
            if (raycastHit2D.collider != null) {
                ...
                if (!colliderList.Contains(raycastHit2D.collider)) {
                    ...
                    if (raycastHit2D.collider.gameObject.tag == "Gear") {
                        // Hit an Item
                        radarPing.SetColor(new Color(0, 1, 0));
                    }
                    if (raycastHit2D.collider.gameObject.tag == "Enemy") {
                        // Hit an Enemy
                        radarPing.SetColor(new Color(1, 0, 0));
                    }
                    if (raycastHit2D.collider.gameObject.tag == "Repair")
                    {
                        // Hit an RepairShip
